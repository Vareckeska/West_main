...
public class APXUtils {
    ...
    public static Resource resGob = Resource
            .load("paginae/union/gobcolor"); // add
    public static Resource resConf = Resource
            .load("paginae/union/Conf"); // add
    public static Resource resAuto = Resource
            .load("paginae/union/autoaggro");
    ...
    public static Resource resGC = Resource
            .load("paginae/union/GC"); //inventory

	public static void addMenu() {
		JSScriptInfo.RemoveAllSriptsFromMenu();
		unionRoot = new MenuElement(null,
				new String[] { CustomMenu.menu_prefix }, resUnion, "West",
				"JBot functions", '*', "union_root");
		union_menu.setRootElement(unionRoot);
        westUtil = new MenuElement(null,
                new String[] { CustomMenu.menu_prefix }, resGC, "Zest",
                "West Util", '*', "west_util");
        union_menu.setRootElement(westUtil);
		/*Здесть насрал Керриган*/
		buddyElement = new MenuElement(null, new String[] {CustomMenu.menu_prefix, "buddy_listener"}, buddyRes,
				"Buddy window", "Toggles buddy window", '*', "buddybtn");
		...
		equipElement = new MenuElement(null, new String[] {CustomMenu.menu_prefix, "equip_listener"}, equipRes,
				"Equipment window", "Toggles equipment window", '*', "equipbtn");
		...
		charElement = new MenuElement(null, new String[] {CustomMenu.menu_prefix, "char_listener"}, charRes,
				"Character window", "Toggles character sheet window", '*', "charbtn");
		...
		optElement = new MenuElement(null, new String[] {CustomMenu.menu_prefix, "opt_listener"}, optRes,
				"Options window", "Toggles options window", '*', "optbtn");
		...
		invElement = new MenuElement(null, new String[] {CustomMenu.menu_prefix, "inv_listener"}, invRes,
				"Inventory window", "Toggles inventory window", '*', "invbtn");
		...
		scriptRootNew = addResource("script_start", "Run Scripts",
				"You can find all availiable scripts in this menu", '*',
		...
		scriptRootRem = addResource("script_stop", "Stop Scripts",
				"You can stop scripts from this menu", '*', resScript1,
				unionRoot, null);

		addResource("toggle_draw_pf_map", "Toggle PF Map", "Do it lol!", '*',
		...
		addResource("stop_all_scripts", "Stop all scripts",
				"Stops all working scripts", '*', resScript5, unionRoot,
				new MenuElemetUseListener(null) {
					public void use(int button) {
		...
		addResource("hide_all_objects", "Hide ALL Objects", "Do it lol!", '*', resScript2, unionRoot,
				new MenuElemetUseListener(null) {
					public void use(int button) {
		...
		addResource("stop_self_moving", "Stop Movement",
				"Stops movement immidietly", '*', resScript2, unionRoot,
				new MenuElemetUseListener(null) {
					public void use(int button) {
						_add_stop_movement();
					}
        ...
        addResource("Gob_Color", "Shows Gob Color",
                "Shows Gob Color", '*', resGob, westUtil,
                new MenuElemetUseListener(null) {
                    public void use(int button) {
                        Config.GobColor = !Config.GobColor;
                        System.out.println("Debugging Gob Color: "+ Config.GobColor);
        ...
        addResource("Auto_Aggro", "AutoAggro",
                "Auto Aggros", '*', resAuto, westUtil,
                new MenuElemetUseListener(null) {


                    public void use(int button) {
                        Config.autoaggro = !Config.autoaggro;
                        System.out.println("Debugging AutoAggro: "+ Config.autoaggro);


                        //write bot window asap, write text updating window for configs on screen in upper hand of screen, check avawindow on how it draws



                    }
                });
        addResource("Config_wind", "Config Window",
                "Shows Configs", '*', resConf, westUtil,
                new MenuElemetUseListener(null) {
                    ...
                    public void use(int button) {
                        if (!cw) {
                            if (CW == null)
                                CW = new ConfigWind(new Coord(300, 10),
                                        //UI.instance.root);
                                        UI.instance.root);
                            CW.show();
                        } else {
        ...
        addResource("Debug_wind", "Debug Console",
                "Shows advanced debug in console", '*', resGC, westUtil,
                new MenuElemetUseListener(null) {
