...
public class MiniMap extends Widget {
    ...
    public void draw(GOut og) {
            ...
            synchronized (simpleTex) {
                for (int y = ulg.y; (y * cmaps.y) - tc.y + (hsz.y / 2) < hsz.y; y++) {
                    for (int x = ulg.x; (x * cmaps.x) - tc.x + (hsz.x / 2) < hsz.x; x++) {
                        ...
                        if (!hidden) {
                            g.image(tex,
                                    cg.mul(cmaps).add(tc.inv()).add(hsz.div(2)));
        ...
        if ((!plx.loading) && (!hidden)) {
            ...
            Coord isz = new Coord(20, 20);
            Coord psz = new Coord(5, 5);
            Coord psz1 = new Coord(2, 2);
            Coord psz2 = new Coord(4, 4);
            Coord c;
            ...
            if (Config.radar) {
                ...
                synchronized (ui.sess.glob.oc) {
                    for (Gob gob : ui.sess.glob.oc) {
                        ...
                        if (gob.isHighlight()
                                && Config.highlightItemList.contains(name)) {
                            Tex tx = Config.hlcfg.get(name).geticon();
                            g.aimage(tx, c, isz, 0.5, 0.5);
                        }
                        ...
                        if (gob.isHuman()) {
                            ...
                            g.fellipse(c, psz2);
                            // g.frect(c.sub(2, 2), c.add(2,2));
                            g.chcolor(Color.BLACK);
                            g.fellipse(c, psz1);
                            g.chcolor();
                        }

                        if (name.contains("kritter/bram")) {
                            g.chcolor(Color.YELLOW);
                            g.fellipse(c, isz.div(2));
                            g.chcolor();
                            Tex tx = Resource.loadtex("paginae/build/bram");
                            g.aimage(tx, c, isz, 0.5, 0.5);
                            g.chcolor();
                        }
