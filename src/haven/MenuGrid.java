	...
	public void use(Resource r) {
		...
		if (getSubResources(r).length > 0) {
			...
			if (actions[0].equals("@")) {
                ...
                for (int i = 0; i < actions.length; i++) { // new
                    if (actions[i].contains("atk")) {
                        if (doubleTapAttack(actions))
                            return;
                    }
                }
				//
                ...
                if (right_click_pressed == h) {
                    if (right_click_pressed.layer(Resource.action) != null) {
                        ...
                        if (actions != null && actions.length > 0 && actions[0].equals(CustomMenu.menu_prefix)) {
                            if (actions.length > 1) {
                                String id = actions[1];
                                CustomMenu.MenuElemetUseListener listener = listeners.get(id);
                                if (listener != null) {
                                    listener.use(button);
                                }
                            }
