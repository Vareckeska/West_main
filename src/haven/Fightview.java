...
public class Fightview extends Widget {
    ...
    public void uimsg(String msg, Object... args) {
        ...
        if (msg == "new") {
            ...
            lsrel.addFirst(rel);
            RootWidget.EnIds.add(rel.gobid);

            Audio.play(Resource.load("sfx/fightstart"));
            ...
            rel.remove();
            lsrel.remove(rel);
            if (RootWidget.EnIds.get(RootWidget.EnId) == rel.gobid)
                RootWidget.EnId = 0;
            int curenid = RootWidget.EnIds.get(RootWidget.EnId);

            for (int i = 0; i < RootWidget.EnIds.size(); i++) {
                if (RootWidget.EnIds.get(i) == rel.gobid) {
                    RootWidget.EnIds.remove(i);
                    break;
                }
            }
            for (int i = 0; i < RootWidget.EnIds.size(); i++) {
                if (RootWidget.EnIds.get(i) == curenid) {
                    RootWidget.EnId = i;
                    break;
                }
            }
            ...
            try {
                ...
                RootWidget.EnId = 0;
                for (int i = 0; i < RootWidget.EnIds.size(); i++) {
                    if (RootWidget.EnIds.get(i) == current.gobid) {
                        RootWidget.EnId = i;
                        break;
                    }
                }
    ...
    public void currentUp() { // new
        if (lsrel.size() > 1) {
            ...
            int playergob =ui.mainview.playergob;
            Coord pc = ui.mainview.glob.oc.getgob(playergob).getc();
            ui.mainview.wdgmsg("click", pc, pc, 1, ui.modflags(), playergob, pc);
