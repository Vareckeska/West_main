...
public class Widget {
    ...
    public void wdgmsg(Widget sender, String msg, Object... args) {
        if (msg.equals("act") && args[0].toString().equals("bp")
                && args[1].toString().equals("hearth")
                && RootWidget.Qpress2 == false) {
            if (ui.mainview != null && ui.menugrid != null) {
                Moving m;
                Gob gob = ui.sess.glob.oc.getgob(ui.mainview.playergob);
                m = gob.getattr(Moving.class);
                if ((m != null) && (m instanceof LinMove)) {
                    RootWidget.Qpress2 = true;
                    Thread myThready = new Thread(new Runnable() {
                        public void run() {
                            Gob gob = ui.sess.glob.oc
                                    .getgob(ui.mainview.playergob);
                            ...
                            if ((m != null) && (m instanceof LinMove)) {
                                lm = (LinMove) m;
                                Coord c = new Coord();
                                c = Coord.sc(lm.t.angle(gob.getc()), 100);
                                c = c.mul(-1, 1);
                                c = c.mul(6.5);
                                c = c.div(100);
                                ui.cons.out.println(c);
                                ui.mainview.wdgmsg("place", gob.getc().sub(c),
                                        1, ui.modflags());

                                try {
                                    Thread.sleep(25);
                                    ui.mainview.wdgmsg("place",
                                            gob.getc().sub(c), 1, ui.modflags());
                                    Thread.sleep(110);
                                } catch (InterruptedException e) {
                                    ui.cons.out.println("catched :(");
                                }
                                ui.mainview.wdgmsg("click", lm.t, lm.t, 1,
                                        ui.modflags());
                                ui.cons.out.println("GOGOGO");
                    ...
                    myThready.start();
                    ui.cons.out.println("place sign");
                }
            }
        }

        /*if (msg.equals("act") && args[0].toString().equals("atk")) {

            if (message_timer + 500 >= System.currentTimeMillis()
                    && message_timer + 50 <= System.currentTimeMillis()) {
                if (RootWidget.currentAttack <= 0) {
                    RootWidget.Qpress = true;
                }
            }
            message_timer = System.currentTimeMillis();
        }*/
        if (RootWidget.Qpress == true) {
            message_timer -= 1000;
            RootWidget.Qpress = false;
            Gob gob = ui.sess.glob.oc.getgob(RootWidget.EnIds
                    .get(RootWidget.EnId));
            ui.cons.out.println("Qpress-ing");
            if (gob != null) {
                if (ui.mainview != null && ui.menugrid != null) {
                    ui.mainview.wdgmsg("click", gob.getc(), gob.getc(), 1,
                            ui.modflags(), gob.id, gob.getc());
                    ui.cons.out.println("Click-ing ui.mainview");
                }
            }
