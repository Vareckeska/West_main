...
public class RootWidget extends ConsoleHost {
	...
	Profile gprof;
	boolean afk = false;
    public static ArrayList<Integer> EnIds = new ArrayList<Integer>();
	public static boolean screenshot = false;
	public static boolean names_ready = false;
    public static boolean Qpress = false;
    public static boolean Qpress2 = false;
	...
	public boolean globtype(char key, KeyEvent ev) {
		if (!super.globtype(key, ev)) {
			...
			if (Config.profile && (key == '`')) {
			...
			}else if ((key == '`') || (key == '~') || key == 1105
                    || key == 1025) {
                // ui.mainview.wdgmsg("click",Coord.fake,ui.sess.glob.oc.getgob(ui.mainview.playergob).getc(),1,
                // 0);
                ui.menugrid.wdgmsg("act", "theTrav", "hearth");
                String str = "Stop moving -> Go HF";
                ...
                ui.cons.out.println("F> " + Integer.toString(EnId) + " "
                        + Integer.toString(EnIds.get(EnId)));
                ...
                ui.cons.out.println("D> " + Integer.toString(EnId) + " "
                        + Integer.toString(EnIds.get(EnId)));

            } else if ((code == KeyEvent.VK_A) && ctrl) {
				Config.autoaggro = !Config.autoaggro;
                String str = "AutoAggro " + ((Config.autoaggro) ? "ON" : "OFF");
                ui.cons.out.println(str);
                ui.slenhud.error(str);
