...
public class Avaview extends Widget {
    ...
    public void draw(GOut g) {
        ...
        if (avagob != 0 && RootWidget.EnIds.get(RootWidget.EnId) == avagob) {
            g.chcolor(255, 0, 0, 192);
            // g.image(Equipory.bg, new Coord(Equipory.bg.sz().x / 2 - asz.x /
            // 2, 20).inv());
            // g.line(new Coord(asz.x/2-36, 8), new Coord(asz.x/2-10, 8), 5);
            Tex tx = Resource.loadtex("paginae/rzn/rzagro");
            g.image(tx, new Coord(asz.x / 2 - 36, 0));
            g.chcolor();
