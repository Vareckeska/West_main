...
public class MapView extends Widget implements DTarget, Console.Directory {
	...
	Camera cam;
    private static final Resource rsrcAlarm = Resource.load("sfx/fightstart", 1);
    ...
    double[] scales = { 0.25D, 0.5D, 0.66D, 0.8D, 0.9D, 1.0D, 1.25D, 1.5D, 1.75D, 2.0D, 3.0D, 5.0D, 10.0D };
	...
	public boolean mousedown(Coord c, int button) {
		...
		if ((cam != null) && cam.click(this, c, mc, button)) {
			...
			//Kerri
			if (button == 3 && ui.modctrl && ui.modmeta) {
    ...
    public boolean mousewheel(Coord c, int amount) {
        if (ui.modshift) {
            if (amount > 0) {
                ...
                RootWidget.EnId += 1;
                if (RootWidget.EnId >= RootWidget.EnIds.size())
                    RootWidget.EnId = 0;
                RootWidget.Qpress = false;

                ui.cons.out.println("F> "
                        + Integer.toString(RootWidget.EnId)
                        + " "
                        + Integer.toString(RootWidget.EnIds
                        .get(RootWidget.EnId)) + ":"
                ...
                RootWidget.EnId -= 1;
                if (RootWidget.EnId < 0)
                    RootWidget.EnId = RootWidget.EnIds.size() - 1;
                RootWidget.Qpress = false;

                ui.cons.out.println("D> "
                        + Integer.toString(RootWidget.EnId)
                        + " "
                        + Integer.toString(RootWidget.EnIds
                        .get(RootWidget.EnId)) + ":"
        ...
        this.si = Math.min(this.scales.length - 1, Math.max(0, this.si - amount));
        setScale(this.scales[this.si]);
        return true;
    }
	...
	/*private void drawObjectRadius(GOut g) {
		...
		synchronized (glob.oc) {
			for (Gob tg : glob.oc) {
                ...
                if(tg.isHuman() && !ui.sess.glob.party.memb.keySet().contains(tg.id)){

                    //g.chcolor(255, 0, 255, 96);
                    //if(gob.id == RootWidget.EnId)
...
*/
               // }
                ...
                /*if (tg.isHighlight()
                        && Config.highlightItemList.contains(tg.resname())) {
                    g.chcolor(255, 128, 64, 96);
                    drawradius(g, tg.sc, 5);
                }*/
    ...
    private void autoaggro(Gob gob) {
        if (!playersSeenAggro.contains(gob.id)) {
            ...
            if (kin == null && Config.aggrounknown || kin != null && kin.group == 2 && Config.aggrored) {
                ...
                if (ui.menugrid != null) {
                    ui.menugrid.wdgmsg("act", new Object[] { "atk", "pow" });
		...
		if (Config.showHidden && Config.hide) {
			...
			synchronized (glob.oc) {
				for (Gob gob : glob.oc) {
                    ...
                    if (res.name.contains("/plants/")){
                        g.chcolor(0, 128, 0, 96);
                    }
    ...
    private boolean contain_ids_in_list(int id) {
        for (int i = 0; i < RootWidget.EnIds.size(); i++) {
            if (RootWidget.EnIds.get(i) == id) {
                return true;
            }
        }
        return false;
    ...
    private void draw_rizen(GOut g) {
        if (Config.showAURA)
            for (Sprite.Part part : clickable) {
                Gob og = (Gob) part.owner;
                if ((part.owner != null) && (part.owner instanceof Gob)
                        && (contain_ids_in_list(og.id) || og.isHuman())) {
                    Color show = null;
                    g.chcolor(0, 0, 0, 255);
                    if (ui.sess.glob.party.memb.keySet().contains(og.id))
                        show = Utils.clipcol(0, 255, 0, 255);
                    if (playergob == og.id)
                        show = Utils.clipcol(255, 255, 255, 128);
                    if (contain_ids_in_list(og.id))
                        show = Utils.clipcol(255, 128, 0, 255);
                    if (og.id == RootWidget.EnIds.get(RootWidget.EnId))
                        show = Utils.clipcol(255, 0, 0, 255);
                    if (show != null) {
                        g.chcolor(show);
                        part.drawol(g);
                        g.chcolor();
    ...
    private void draw_rtarget(GOut g) {
        synchronized (glob.oc) {
            if (target_cross == null) {
                BufferedImage buf = TexI.mkbuf(new Coord(40, 20));
                Graphics2D g2d = buf.createGraphics();
                g2d.setStroke(new BasicStroke(3.f)); // set stroke width of 3
                g2d.setColor(Color.RED);
                g2d.drawLine(m2s(new Coord(-5, -5)).add(new Coord(20, 10)).x,
                        m2s(new Coord(-5, -5)).add(new Coord(20, 10)).y,
                        m2s(new Coord(5, 5)).add(new Coord(20, 10)).x,
                        m2s(new Coord(5, 5)).add(new Coord(20, 10)).y);
                g2d.drawLine(m2s(new Coord(5, -5)).add(new Coord(20, 10)).x,
                        m2s(new Coord(5, -5)).add(new Coord(20, 10)).y,
                        m2s(new Coord(-5, 5)).add(new Coord(20, 10)).x,
                        m2s(new Coord(-5, 5)).add(new Coord(20, 10)).y);
                g2d.dispose();
                target_cross = new TexI(buf);
            }
            if (RootWidget.EnIds.get(RootWidget.EnId) != 0) {
                Gob targid = ui.sess.glob.oc.getgob(RootWidget.EnIds
                        .get(RootWidget.EnId));
                if (targid != null)
                    if (targid.sc != null) {
                        Coord ct = targid.sc;
                        g.chcolor();
                        g.image(target_cross,
                                ct.add(target_cross.sz().div(2).inv()));
                    }
            }
        }
    }
	...
	public void draw(GOut og) {
        ...
        /*if (Config.autoaggro = true){
            ui.slenhud.error("AutoAggro on");
        }
        if (Config.autoaggro = false){
            ui.slenhud.error("AutoAggro off");
        }*/
        //ConfigWind.AA.settext("GobColor: " + Config.GobColor);
        if (Config.CWind == true) {
            try {
                ConfigWind.AA.settext("autoaggro: " + Config.autoaggro);
                ConfigWind.GC.settext("GobColor: " + Config.GobColor);
                ConfigWind.DB.settext("Debuging: " + Config.debug);
            } catch (Exception e) {
                e.printStackTrace();
            }
                        ...
                        if (kins == null)
                            ...
                            switch (kins.group) {
                                ...
                                case 3:
                                    ...
                                    break;
                                case 4:
                                    g.chcolor(51, 255, 204, 255);
                                    break;
                                case 5:
                                    g.chcolor(255, 255, 0, 255);
                                    break;
                                case 6:
                                    g.chcolor(255, 0, 255, 255);
                                    break;
                                case 7:
                                    g.chcolor(255, 150, 50, 255);
                                    break;
                            }
                    } else
                        g.chcolor(Color.ORANGE);
                    if ((Config.showpath && gob.isHuman())
                            || (Config.showpathAll && !gob.isHuman())) {
                        if (gob.sc == null) {
                            continue;
                        }
    ...
    /*public void createConfig() {
        ...
        if (CW == null){
            CW = new ConfigWind(new Coord(300, 20), this.parent);
        }
        if (Config.CWind) {
            CW.show();
            //Numenb.update();
